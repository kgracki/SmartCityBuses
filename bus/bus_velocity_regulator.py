...
class BusVelocityRegulator:
    ...
    def calculate_new_bus_velocity(current_bus_navigator: BusNavigator, desired_distance, max_velocity, max_velocity_change, neighbours_info):
        ...
        distance_to_next_bus = current_bus_navigator.calculate_distance_between(current_bus_position, next_bus_position)
        distance_to_previous_bus = current_bus_navigator.calculate_distance_between(current_bus_position, previous_bus_position)
        ...
        if distance_to_next_bus > 0 and distance_to_previous_bus < 0:
            factor = (distance_to_next_bus - desired_distance) / desired_distance
        # it means that next bus is very, very daleko
        elif distance_to_next_bus < 0:
            factor = 1
